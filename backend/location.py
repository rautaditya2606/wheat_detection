...
def reverse_geocode(lat, lon):
    ...
    try:
        ...
        headers = {"User-Agent": "WheatDiseaseDetection/1.0 (your@email.com)"}
        ...
        return {"formatted_address": formatted_address, "details": address}
    except requests.exceptions.RequestException as e:
...
def update_user_location(user, location_data, source):
    ...
    if not user or not user.is_authenticated:
        flask_current_app.logger.error("Cannot update location: User not authenticated")
        return None
...
def update_location():
    ...
    try:
        ...
        user = update_user_location(
            current_user._get_current_object(), location_data, "browser"
        )
        ...
        return (
            jsonify(
                {"status": "error", "message": "Invalid location data", "error": str(e)}
            ),
            400,
        )
    except Exception as e:
        flask_current_app.logger.error(
            "Location update error: %s", str(e), exc_info=True
        )
        return (
            jsonify(
                {
...
def update_location_consent():
    ...
    try:
        ...
        user = user_db.update_user(current_user.id, location_consent=data["consent"])

        if not user:
            return jsonify({"status": "error", "message": "User not found"}), 404

        return jsonify({"status": "success", "consent": user.location_consent})

    except Exception as e:
        flask_current_app.logger.error(
            "Consent update error: %s", str(e), exc_info=True
        )
        return (
            jsonify(
                {
